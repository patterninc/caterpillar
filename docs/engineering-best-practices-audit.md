...
A previous audit dated 2026-09-09 was merged in PR #119 and reverted in PR #122, so no report currently exists in the tree. Re-auditing against the current `main` (latest release `v2.30.0`), **no checklist item changed status** since that audit: CI still runs only `go build`, there is still no `AGENTS.md`, and the branch rulesets are unchanged. Verdicts, counts, and scores below therefore match the 2026-09-09 report, with evidence refreshed to the current tree.

## Repo profile

`caterpillar` is a Go 1.25 data-ingestion and pipeline-processing engine owned by Pattern (`patterninc/caterpillar`, verified via `gh repo view`). It ships as a distributable CLI binary and a multi-arch Docker image published to Docker Hub (`patternoss/caterpillar`) by `.github/workflows/release.yaml`; the repo itself deploys no hosted service — no Terraform, no ECS/Lambda/Kubernetes configs, no owned database. Package layout follows the Go idiom (`cmd/caterpillar`, `internal/pkg/pipeline/task/...`), with a per-task `README.md` under 19 of the 20 `task/*` directories (the `aws/` wrapper directory has none; its integration lives in `aws/parameter_store/`). GitHub reports ~15 human contributors, and `.github/CODEOWNERS` assigns everything to `@patterninc/data-acquisition`, so this is a team-scale repo. AWS SDK integrations (SQS, SNS, S3, Parameter Store, Translate) exist as pipeline tasks the *user's* pipelines run against; the repo has no owned AWS footprint, so item 49's `aws[]` sub-check does not apply. There is no browser UI, no visual surface, and no persistence layer. Because ownership is `patterninc` (API-verified), Pattern inherited controls apply to items 19, 20, 39, and 47.
...
Critical gates are RED because item 2 (AGENTS.md) is a Gap and items 16 (required CI) and 40 (scoped secrets) are Partial. Adjusted compliance is calculated independently:
...
| 6 | README with setup & run instructions | **Met** | `README.md` (~390 lines): purpose, prerequisites, build, run, DAG syntax, error handling, dynamic config, supported tasks; `DAG_README.md` covers DAG mode | — |
...
| 9 | CODEOWNERS | **Met** | `.github/CODEOWNERS` — `* @patterninc/data-acquisition`; the `main + releases` ruleset requires code-owner review | — |
...
| 14 | Commit message conventions | **Met** | Recent history uses Conventional Commits consistently (`feat(sqs):`, `fix(kafka):`, `docs:` across recent PRs #119–#125) | — |
| 15 | Branch protection | **Met** | Repo ruleset `main + releases` (id 8564244) requires PR + code-owner review and blocks deletion, non-fast-forward, direct creation, and update on `main` and `release/**`; org ruleset `require-pr-review` (id 3174764) also applies. Critical gate. | — |
| 16 | Required CI checks before merge | **Partial** | `.github/workflows/ci.yaml` runs only `go build` on PRs; no `go test`, no lint; neither ruleset lists a `required_status_checks` rule | Add `go test ./...` and `golangci-lint` to `ci.yaml`, then add a required-status-checks rule to the `main + releases` ruleset. Critical gate. |
| 17 | Dependency allow-lists / deny-lists | **Gap** | No policy config | Add a lightweight allow-list (`depguard` rule) restricting new top-level dependencies. |
...
| 21 | Max complexity limits | **Gap** | No cyclomatic-complexity config | Enable `gocyclo`/`gocognit` under golangci-lint with a starting ceiling. |
| 22 | Import boundary enforcement | **Gap** | `internal/` is used, but no `depguard` or equivalent linter | Enforce package boundaries between `pipeline`, `pipeline/task/*`, and shared helpers via `depguard`. |

## Testing & Feedback Loops

| # | Practice | Status | Evidence | Recommendation / rationale |
|---|----------|--------|----------|----------------------------|
| 23 | Unit tests | **Met** | `internal/pkg/pipeline/dag_test.go` and `internal/pkg/pipeline/ack/ack_test.go` cover the DAG parser and ack primitives (testify). Critical gate. | — |
| 24 | Integration tests | **Met** | `internal/pkg/pipeline/task/acking_test.go` wires multiple tasks end-to-end; `test/pipelines/*_test.yaml` (18 files) are runnable full-pipeline scenarios documented in the README. Critical gate. | — |
| 25 | Snapshot / golden-file tests | **Gap** | No `testdata/` golden directory; `test/pipelines/converter/*_test_results.json` are write targets, not diffed references | Capture expected pipeline outputs as golden files and diff on run. |
| 26 | Contract tests (Pact) | **Not applicable** | No owned network API; integrations (Kafka, SQS, HTTP, SFTP) test the wire directly | Integration runs + per-task READMEs cover the equivalent surface. |
...
| 31 | Load / performance benchmarks | **Gap** | No `Benchmark*` functions anywhere in the tree | Add Go benchmarks for jq/xpath/converter hot paths and record baselines. |
...
| 34 | Deterministic test fixtures | **Met** | Fixed inputs under `test/pipelines/` (`bcrypt_cases.json`, `greetings.json`, `sample.csv`, `planets.tar.gz`, …) | — |
...
| 37 | One-command setup | **Partial** | `makefile` has `all: build test`, but `test` iterates `directories=` which is empty — running `make` effectively builds only | Wire `test` to `go test ./...` (with coverage), and add a `make dev` alias documenting the CGO/librdkafka prerequisite. |
...
| 43 | Structured logging (JSON) | **Gap** | Tasks report via `fmt.Print*` to stdout and `log.Fatal`/`log.Println` (e.g. `internal/pkg/pipeline/task/archive/zip.go`, `task/converter/csv.go`); the README itself notes errors interleave with record output | Switch to `log/slog` with JSON output on stderr, tagging task name, record ID, and pipeline run ID. |
...
| 45 | Feature flags with local overrides | **Gap** | No flag SDK or env-var toggle convention | Add a lightweight flag hook (env-var overrides in dev) for opt-in features like DAG mode, once it exits EXPERIMENTAL. |
...
| 48 | Reproducible builds (lockfiles) | **Met** | `go.mod` + `go.sum` pin all dependencies; `build/Dockerfile` pins `golang:1.24.7-alpine` and Alpine 3.20; CI/release pin Go 1.24.7. Critical gate. | — |

## Documentation & Context (agent dispatch)

| # | Practice | Status | Evidence | Recommendation / rationale |
|---|----------|--------|----------|----------------------------|
| 49 | Agent-dispatch manifest | **Gap** | No `.agents/pattern-agents.json` (or `.yml`/`.yaml`); `backstage.yaml` exists but is not the dispatch manifest | Add `.agents/pattern-agents.json` with `schema_version`, `github.repo: patterninc/caterpillar`, ClickUp list, Slack channel, and Datadog service metadata. `aws[]` may be omitted (no owned AWS footprint). |

## Prioritized recommendations

1. **[S] Gap — AGENTS.md (item 2, critical gate):** Author `AGENTS.md` at repo root covering build (`go build ./cmd/caterpillar/caterpillar.go`), test (`go test ./...` plus the `test/pipelines/*_test.yaml` harness), the pipeline-YAML rules, and the per-task README convention.
2. **[S] Partial — required CI (item 16, critical gate):** Extend `.github/workflows/ci.yaml` to run `go test ./...` and `golangci-lint`, add a required-status-checks rule to the `main + releases` ruleset, and align the CI `go-version` (1.24.7) with the `go.mod` directive (1.25.0).
...
19. **[M] Gap — feature flags (item 45):** Wire a flag mechanism (env-var fallback in dev) for opt-in features like DAG mode.
...
22. **[S] Gap — devcontainer (item 36):** Add `.devcontainer/` pinned to Go + Alpine 3.20 + librdkafka to match `build/Dockerfile`.
23. **[M] Gap — snapshot tests (item 25):** Add golden-file outputs for pipeline runs and diff on run.
...
- Per-task `README.md` files under 19 of the 20 `internal/pkg/pipeline/task/*/` directories (all except the `aws/` wrapper) document each task's config surface — a repeatable convention that gives agents (and humans) a predictable place to look, well beyond what item 6 rewards.
- The `test/pipelines/*_test.yaml` suite is a self-documenting integration harness: each YAML is both an example referenced from the README and a runnable regression scenario, so contributors adding a new task write docs and test in the same file.
...
- Conventional-commit discipline is consistent across releases (30+ tags, latest `v2.30.0`), which makes auto-generated release notes usable without curation.
- The README's error-handling section documents failure semantics (`fail_on_error`, acknowledgment interaction, back-pressure caveat) with unusual candor, including known sharp edges.
