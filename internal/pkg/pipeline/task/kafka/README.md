...
Note: the task sets the reader `QueueCapacity` to the configured `batch_size`.
...
| `batch_size` | int | `100` | Number of messages to buffer/flush for write and reader `QueueCapacity` |
...
## Notes and Limitations
 ...
 - The task uses a single configured `timeout` (default 15s) for dial, read, write and commit operations unless you extend the code to expose per-operation timeouts. Dial attempts use the same `timeout` value for each connection attempt.
