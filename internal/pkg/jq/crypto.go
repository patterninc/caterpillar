...
// Go's bcrypt libraries always draw their own random salt, so hashing against a
...
// A full hash is also a valid salt argument, since its trailing field starts with the salt.
...
// No cryptographic primitive is reimplemented here: the Blowfish calls below are the same
// exported ones the standard bcrypt implementation drives.
